...
def Data_Preprocess(Data, slice_size=1000, L = 120, window=False, random=42 , divide=0.33):
    '''
    ...
    ##-----------When the strides is active
        ...
        for element in data :
            ...
            Y=[]
            for k in range(size):
                X.append(np.array(data[element][int(k*(L)):int(k*L+slice_size)])) ## for each window it strides with windowsize/2
                Y.append(element%10)
            X_ALL.append(X)
            Y_ALL.append(Y)
    ...
    X_train, X_test, y_train, y_test = train_test_split(np.asarray(normalize(FFT_X)),np.asarray(Flat_Y), test_size=divide, random_state=random,shuffle=True)
...
X_train, X_test, y_train, y_test = Data_Preprocess('DataSetA.mat',slice_size=1200, L=120, window=False)
